...
# 🌈 YOUR ORANGE Palette + Glassmorphism
st.markdown("""
<style>
...
def load_data():
    ...
    st.error("⚠️ Put Excel file in repo root and rename to `hyperlocal_demand_forecasting_with_grocery_items-2.xlsx`")
    return pd.DataFrame()
...
# Sidebar: Live Metrics (✅ DATASET ALIGNMENT)
with st.sidebar:
...
with col1:
    ...
    **Neighborhood-level grocery predictions** powered by Prophet ML.
    ...
    # ✅ FIXED: REAL Popper.js (no empty space)
    components.html("""
    <div id="popper-root" style="margin-top: -10px;">
      <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/2.11.8/umd/popper.min.js"></script>
      <button id="metrics-help" class="popper-trigger" style="margin-top: -5px;">
        ℹ️ Metrics Guide
      </button>
      <div id="metrics-tooltip" class="popper-tooltip" style="display:none;">
        <strong>Next Month:</strong> Prophet-predicted demand (₹/month)<br>
        <strong>Days of Cover:</strong> Current stock ÷ daily forecast<br>
        <small>🟢 >7d = Good | 🔴 <3d = Restock Now</small>
      </div>
      <script>
        ...
        function create() {
          popperInstance = Popper.createPopper(trigger, tooltip, {
            placement: "right-start", modifiers: [{name: "offset", options: {offset: [0, 8]}}]
          });
        }
        trigger.onclick = () => {
          const shown = tooltip.style.display !== "none";
          tooltip.style.display = shown ? "none" : "block";
          if (!shown) { if (!popperInstance) create(); popperInstance.update(); }
          else if (popperInstance) { popperInstance.destroy(); popperInstance = null; }
        };
        document.onclick = (e) => {
    ...
    # ✅ FIXED: Cards show properly (no empty space)
    ...
    with col_b:
        ...
        days_cover = 7 / daily_avg * 30  # Fixed calculation
        with st.container():
            st.markdown('<div class="metric-card">', unsafe_allow_html=True)
...
st.markdown("*Production Dashboard | Streamlit Cloud | Python + Prophet ML | v2.8*")
